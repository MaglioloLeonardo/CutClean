...
manifests are published, and the images are obtained from the official CelebA
release, which ``datasets/celebA.py`` downloads on first use.
...
def main():
    ...
    if args.check:
        ...
        print(f"  {'present' if os.path.isdir(celeba) else 'MISSING'}  {'celeba':26s} {celeba}"
              "   (official release, downloaded on first use)")
    ...
    print("[data] CelebA images are not redistributed: they are fetched from the "
          "official release the first time the loader runs.")
    return 0
